...
import { Plus, Search, Star, ChefHat, Clock, Filter } from "lucide-react";
...
export default function KuechePage() {
  ...
  const load = useCallback(async () => {
    ...
    setGerichte(Array.isArray(data) ? data : []);
    setLoading(false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
        >
          <option value="name">Name A–Z</option>
          <option value="name-desc">Name Z–A</option>
          <option value="neueste">Neueste zuerst</option>
          <option value="zeit">Kürzte Zeit</option>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">{search || nurFavoriten ? "Keine Ergebnisse gefunden." : "Noch keine Gerichte. Legen Sie das erste an!"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {gerichte.map((g) => (
            ...
            >
              ...
              <h3 className="font-semibold text-gray-900 dark:text-white pr-8 mb-1 line-clamp-2">{g.name}</h3>
              ...
              {g.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {g.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-xs bg-orange-50 dark:bg-orange-950 text-orange-600 px-2 py-0.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>

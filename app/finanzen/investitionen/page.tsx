    ...
    <div className="space-y-6">
      ...
      {investments.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-blue-50 dark:bg-blue-950 rounded-xl p-4">
            <p className="text-xs text-gray-500">{t("finanzen.invested")}</p>
          ...
          <div className="bg-emerald-50 dark:bg-emerald-950 rounded-xl p-4">
            <p className="text-xs text-gray-500">{t("finanzen.currentValue")}</p>
          ...
          <div className={`rounded-xl p-4 ${profit >= 0 ? "bg-emerald-50 dark:bg-emerald-950" : "bg-red-50 dark:bg-red-950"}`}>
            <p className="text-xs text-gray-500">{t("finanzen.profitLoss")}</p>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-4">
          <h2 className="font-semibold text-gray-800 dark:text-white">{t("finanzen.newInvestment")}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-600 mb-1">{t("finanzen.label")}</label>
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">{t("finanzen.ticker")} ({t("common.optional")})</label>
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">{t("finanzen.investedAmount")} (€)</label>
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">{t("finanzen.currentValue")} (€)</label>
            ...
            <div>
              ...
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="stocks">{t("finanzen.investmentTypes.stocks")}</option>
                <option value="etf">{t("finanzen.investmentTypes.etf")}</option>
                <option value="crypto">{t("finanzen.investmentTypes.crypto")}</option>
                <option value="real-estate">{t("finanzen.investmentTypes.realEstate")}</option>
                <option value="savings">{t("finanzen.investmentTypes.savings")}</option>
                <option value="other">{t("finanzen.investmentTypes.other")}</option>
              </select>
            </div>
            <div>
              <label className="block text-sm text-gray-600 mb-1">{t("finanzen.startDate")}</label>
          ...
          <div className="flex gap-2 justify-end">
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl">{t("common.cancel")}</button>
            <button type="submit" disabled={saving} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-xl disabled:opacity-50">{saving ? t("finanzen.saving") : t("common.save")}</button>
          </div>
        </form>
      )}
            ...
            return (
              <div key={inv._id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 px-4 py-4 flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 capitalize">{inv.type}{inv.ticker ? ` · ${inv.ticker}` : ""} · {new Date(inv.startDate).toLocaleDateString(lang)}</p>
                </div>
